<template>
    ...
    <section class="container div-main-text">
        <div id="divSearchLine">
            ...
            <button data-search-button @input="searchNs()">
        ...
        <!-- 검색결과전용 -->
        <div id="divNews">
            <div v-for="subItem in resultList" class="news-item">
                <router-link :to="{name: 'NewsDetail', params: {id: subItem.number}}"><!-- 반복구간 시작 -->
                    <div class="div-item">
                        <img data-item-img :src="subItem.thumImg" alt="">
                        <ul>
...
<script setup>
    ...
    async function searchNs() {
        ...
        const allWr = document.getElementById('divNews');

        if ( srchValue != null ) {
            resultList.value = newsGroup.value.children.filter((f) => f.title.toString().toUpperCase().includes(srchValue) || f.textAll.toString().toUpperCase().includes(srchValue))
